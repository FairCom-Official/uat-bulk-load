...
databases**, then shows you the results side by side.

- **MariaDB** loads the data using its built-in `LOAD DATA LOCAL INFILE` command.
- **FairCom Edge** loads the **exact same data** using its **REST API** (the
  `insertRecords` action), because FairCom is API-first and has no equivalent
  bulk-file command.

Both databases run in Docker on your own machine and load the identical file, so
the comparison is fair.

> **What you do NOT need to know:** SQL, Python, or how either database works.
> If you can copy and paste one command, you can run this.
...
1. **Docker Desktop** — runs the two databases.
   Download: <https://www.docker.com/products/docker-desktop/>
   After installing, **open Docker Desktop and wait until it says "Running."**
2. **Python 3** — runs the loader scripts.
   Check by typing `python3 --version` in a terminal. If you get a version
   number, you're good. If not, install from <https://www.python.org/downloads/>.
3. **This project folder** on your computer (you already have it).

> You do **not** need to create a Python "virtual environment." This tool uses
> your system Python directly.
...
- creates your settings file (`.env`) if it doesn't exist,
...
This second report exists to help the engineering team understand **where
FairCom spends its time** when loading over the REST API, so the loading process
can be improved. It breaks the load into phases and shows per-request timing:
...
In plain terms: almost all the time is the **server processing each request**,
not the network or the Python code — which tells engineering where to focus.

---

## Changing the settings

All settings live in a file called `.env` (it ships with the project). Open it
in any text editor and change the values.
...
After changing `.env`, just run `./run.sh` again. If you change `ROW_COUNT`, the
tool automatically regenerates the data file to match.
...
The two `sweep_*` scripts accept custom values, e.g.
...
- **FairCom evaluation license:** the FairCom Edge container runs for **3 hours**
  at a time on the free evaluation license. If a run fails because FairCom
  stopped, just restart it with `docker compose up -d faircom-edge` and try
  again.
- **FairCom login:** the REST API is at `http://localhost:8080/api`, with
  username/password `ADMIN` / `ADMIN`.
- **MariaDB image:** by default this uses the public `mariadb:11.8` image so it
  works out of the box. To use the production
...
For the full technical details (FairCom API request formats, schema, design
decisions), see [.github/copilot-instructions.md](.github/copilot-instructions.md).
